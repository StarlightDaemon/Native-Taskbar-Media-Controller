...
typedef HWND(WINAPI* pCreateWindowInBand)(DWORD dwExStyle, LPCWSTR lpClassName, LPCWSTR lpWindowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hWndParent, HMENU hMenu, HINSTANCE hInstance, LPVOID lpParam, DWORD dwBand);
...
struct ModSettings
{
  ...
  bool pauseOnNewMediaPlayed = true;
  bool resumeDefaultMediaIfNothingIsPlayed = false;
} g_Settings;
...
// g_currentMedia{nullptr};
std::optional<winrt::Windows::Media::Control::GlobalSystemMediaTransportControlsSession> g_previousMedia;
std::optional<winrt::Windows::Media::Control::GlobalSystemMediaTransportControlsSession> g_DefaultMediaSession;
...
void LoadSettings()
{
  ...
  g_Settings.pauseOnNewMediaPlayed = Wh_GetIntSetting(L"PauseOnNewMediaPlayed");
  g_Settings.resumeDefaultMediaIfNothingIsPlayed = Wh_GetIntSetting(L"ResumeDefaultMediaIfNothingIsPlayed");
}
...
void UpdateMediaInfo()
{
  try
  {
    ...
    g_NumOfMedia = 0;
    winrt::Windows::Foundation::Collections::IVectorView<winrt::Windows::Media::Control::GlobalSystemMediaTransportControlsSession> sessionsList = g_SessionManager.GetSessions();
    for (auto const& session : sessionsList)
    {
      if (session)
      {
        ...
        {
          if (!g_Settings.multipleMediaControl && pb.PlaybackStatus() != GlobalSystemMediaTransportControlsSessionPlaybackStatus::Playing)
          {
            continue;
          }
          ...
          if (newTitle != g_MediaStates[g_NumOfMedia].title || g_MediaStates[g_NumOfMedia].albumArt == nullptr)
          {
            ...
            if (thumbRef)
            {
              ...
              g_MediaStates[g_NumOfMedia].albumArt = StreamToBitmap(stream);
            }
          }
          ...
          if (g_Settings.pauseOnNewMediaPlayed && info.PlaybackStatus() == GlobalSystemMediaTransportControlsSessionPlaybackStatus::Playing && g_MediaStates[g_NumOfMedia].isPlaying == false)
          {
...
{
  if (g_Settings.multipleMediaControl && g_Settings.resumeDefaultMediaIfNothingIsPlayed)
  {
    auto sessionsList = g_SessionManager.GetSessions();
    for (int i = 0; i < g_NumOfMedia; i++)
    {
      if (auto session = sessionsList.GetAt(i))
      {
        if (g_MediaStates[i].isMouseOverArt)
        {
          g_MediaStates[i].isDefaultMedia = !g_MediaStates[i].isDefaultMedia;  // toogle default media
...
void DrawMediaPanel(HDC hdc, int width, int height)
{
  ...
  for (int i = 0; i < g_NumOfMedia; i++)
  {
    if (g_MediaStates[i].isPlaying)
    {
      ...
      playingState.albumArt = g_MediaStates[i].albumArt ? g_MediaStates[i].albumArt->Clone() : nullptr;
    ...
    if (g_MediaStates[i].hasMedia)
    {
      ...
      currentMedia.albumArt = g_MediaStates[i].albumArt ? g_MediaStates[i].albumArt->Clone() : nullptr;
      GraphicsPath path;
      ...
      if (currentMedia.albumArt)
      {
        ...
        // Draw Art
        graphics.DrawImage(currentMedia.albumArt, artX, artY, artSize, artSize);
        if (g_Settings.multipleMediaControl)
        {
  ...
  if (g_HoverState == 1)
    graphics.FillEllipse(&activeBg, pX - circleR, (float)controlY - circleR, circleR * 2, circleR * 2);
  ...
  float plX = pX + gap;
  if (g_HoverState == 2)
    graphics.FillEllipse(&activeBg, plX - circleR, (float)controlY - circleR, circleR * 2, circleR * 2);
  ...
  float nX = plX + gap;
  if (g_HoverState == 3)
    graphics.FillEllipse(&activeBg, nX - circleR, (float)controlY - circleR, circleR * 2, circleR * 2);
...
void CALLBACK TaskbarEventProc(HWINEVENTHOOK, DWORD event, HWND hwnd, LONG, LONG, DWORD, DWORD)
{
...
LRESULT CALLBACK MediaWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
  switch (msg)
  {
    ...
    case WM_MOUSEMOVE:
    {
      ...
      int startControlX = (g_artPadding + artSize) * g_NumOfMedia + (int)(g_artPadding * 2 * scale);
      float gap = 28.0f * (float)scale;
      float pX = (float)startControlX;
      float plX = pX + gap;
      float nX = plX + gap;
